...
export default function LaunchPage() {
  ...
  async function loadAssets() {
    ...
    const [{ data: accs }, { data: pxls }, { data: pgs }] = await Promise.all([
      supabase.from('ad_accounts').select('account_id,account_name,is_selected').eq('user_id', user.id),
      supabase.from('pixels').select('pixel_id,pixel_name,is_selected').eq('user_id', user.id),
      supabase.from('facebook_pages').select('page_id,page_name,is_selected').eq('user_id', user.id),
    ])
    setAdAccounts((accs||[]).map((a: {account_id:string;account_name:string}) => ({ id: a.account_id, name: a.account_name, value: a.account_id })))
    setPixels((pxls||[]).map((p: {pixel_id:string;pixel_name:string}) => ({ id: p.pixel_id, name: p.pixel_name, value: p.pixel_id })))
    setPages((pgs||[]).map((p: {page_id:string;page_name:string}) => ({ id: p.page_id, name: p.page_name, value: p.page_id })))
  ...
  function validate(): string {
    ...
    if (!form.adAccountId) return 'Selecciona un Ad Account.'
  ...
  async function handleLaunch() {
    ...
    if (!res.ok) { addLog('ERROR', result.error || 'Error desconocido'); setError(result.error || 'Error al crear la campaña.'); setLaunching(false); return }
    result.results?.forEach((r: {adSetName:string}) => addLog('SUCCESS', `✓ Creado: ${r.adSetName}`))
    addLog('SUCCESS', `🎉 Campaña "${result.campaignName}" creada. ${result.adsCreated} ad sets en PAUSED.`)
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="card bg-emerald-900/10 border-emerald-700/40 text-center py-8 mb-5">
          ...
          <p className="text-gray-500 text-xs mb-6">Revisa en Meta Ads Manager y actívalos cuando estés listo.</p>
          <div className="flex gap-3 justify-center">
            <a href="https://adsmanager.facebook.com" target="_blank" className="btn-primary text-xs py-2 px-5">VER EN META ADS →</a>
            <button onClick={() => { setLaunchDone(false); setLogs([]); setVideos([]); setShowPreview(false) }} className="btn-secondary text-xs">NUEVO LANZAMIENTO</button>
          </div>
        </div>
        <div className="card">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        {error && (
          <div className="bg-red-900/20 border border-red-700/40 rounded-xl p-4 flex gap-3">
            ...
            <div className="flex-1"><div className="text-red-300 text-sm font-medium">Error</div><div className="text-red-400 text-xs mt-0.5">{error}</div></div>
        ...
        <div className="card">
          ...
          {adAccounts.length === 0 && <div className="bg-amber-900/20 border border-amber-700/30 rounded-lg p-3 text-xs text-amber-300 mb-3">No hay assets. Ve a <a href="/connect" className="underline">Conectar Meta</a> primero.</div>}
          <div className="grid grid-cols-3 gap-3">
            <div><label className="block text-xs text-gray-500 mb-1.5 tracking-wider uppercase">Ad Account</label>
            ...
            <div><label className="block text-xs text-gray-500 mb-1.5 tracking-wider uppercase">Pixel</label>
            ...
            <div><label className="block text-xs text-gray-500 mb-1.5 tracking-wider uppercase">Facebook Page</label>
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div><label className="block text-xs text-gray-500 mb-1.5 tracking-wider uppercase">Nombre del producto *</label>
              <input value={form.productName} onChange={e => set('productName', e.target.value)} className="input-field" placeholder="ECHOFREE" /></div>
            <div><label className="block text-xs text-gray-500 mb-1.5 tracking-wider uppercase">País *</label>
          ...
          <div className="mb-3"><label className="block text-xs text-gray-500 mb-1.5 tracking-wider uppercase">URL de destino *</label>
            <input value={form.destinationUrl} onChange={e => set('destinationUrl', e.target.value)} className="input-field" placeholder="https://mitienda.com/producto" type="url" /></div>
          <div className="grid grid-cols-3 gap-3">
            <div><label className="block text-xs text-gray-500 mb-1.5 tracking-wider uppercase">Presupuesto/día por adset ($)</label>
              <input value={form.dailyBudget} onChange={e => set('dailyBudget', e.target.value)} className="input-field" type="number" min="1" /></div>
            <div><label className="block text-xs text-gray-500 mb-1.5 tracking-wider uppercase">Evento de conversión</label>
            ...
            <div><label className="block text-xs text-gray-500 mb-1.5 tracking-wider uppercase">Call to Action</label>
        ...
        <div className="card">
          ...
          <div className="mb-3"><label className="block text-xs text-gray-500 mb-1.5 tracking-wider uppercase">Texto principal *</label>
            <textarea value={form.primaryText} onChange={e => set('primaryText', e.target.value)} className="input-field" rows={3} placeholder="¿Cansado del ruido? Descubre EchoFree..." /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-xs text-gray-500 mb-1.5 tracking-wider uppercase">Titular *</label>
              <input value={form.headline} onChange={e => set('headline', e.target.value)} className="input-field" placeholder="Duerme Sin Interrupciones" /></div>
            <div><label className="block text-xs text-gray-500 mb-1.5 tracking-wider uppercase">Descripción (opcional)</label>
              <input value={form.description} onChange={e => set('description', e.target.value)} className="input-field" placeholder="Envío gratis hoy" /></div>
          </div>
        </div>
        <div className="card">
          <div className="card-title">Targeting</div>
          <div className="grid grid-cols-3 gap-3 mb-3">
            <div><label className="block text-xs text-gray-500 mb-1.5 tracking-wider uppercase">Edad mínima</label>
              <input value={form.ageMin} onChange={e => set('ageMin', e.target.value)} className="input-field" type="number" min="18" max="65" /></div>
            <div><label className="block text-xs text-gray-500 mb-1.5 tracking-wider uppercase">Edad máxima</label>
              <input value={form.ageMax} onChange={e => set('ageMax', e.target.value)} className="input-field" type="number" min="18" max="65" /></div>
            <div><label className="block text-xs text-gray-500 mb-1.5 tracking-wider uppercase">Género</label>
          ...
          <div className="bg-gray-800/50 rounded-lg px-3 py-2 text-xs text-gray-500">✓ Advantage+ Placements activado por defecto</div>
        </div>
        <div className="card">
          <div className="card-title">Videos <span className="text-gray-600 text-xs font-normal ml-1">— {videos.length}/10</span></div>
          <div className="border border-dashed border-gray-700 rounded-xl p-8 text-center cursor-pointer hover:border-blue-500 hover:bg-blue-500/5 transition-all mb-3"
            onClick={() => fileRef.current?.click()}
            onDragOver={e => e.preventDefault()}
            onDrop={e => { e.preventDefault(); handleFiles(e.dataTransfer.files) }}>
            <div className="text-3xl mb-2">📹</div>
            <div className="text-sm text-gray-400 mb-1">Arrastra los videos aquí o haz click</div>
            <div className="text-xs text-gray-600">MP4, MOV, AVI · Máx 4GB · Hasta 10 videos</div>
          </div>
          ...
          {videos.length > 0 && <div className="space-y-2">
            {videos.map((v, i) => (
              <div key={i} className="flex items-center gap-3 bg-gray-800 rounded-lg px-3 py-2">
                ...
                <button onClick={() => removeVideo(i)} className="text-gray-600 hover:text-red-400 text-sm">✕</button>
              </div>
            ))}
          </div>}
        </div>
        {showPreview && videos.length > 0 && form.productName && (
          <div className="card border-blue-800/50">
            ...
            <div className="space-y-2 mb-4">
              {[['Nombre de campaña', names.campaign,'text-blue-400'],['Ad sets a crear',String(videos.length),''],['Presupuesto total / día',`$${totalBudget} USD`,'text-emerald-400'],['URL con UTMs',finalUrl.slice(0,60)+'...','text-xs'],['Estado inicial','PAUSED','']].map(([k,v,cls]) => (
                <div key={k} className="flex justify-between items-center py-2 border-b border-gray-800 text-xs">
                  <span className="text-gray-500">{k}</span>
                  <span className={`font-medium ${cls} max-w-xs text-right`}>{k==='Estado inicial'?<span className="badge-paused">{v}</span>:v}</span>
                </div>
              ))}
            </div>
        ...
        <button onClick={!showPreview && !launching ? () => { const e = validate(); if(e){setError(e);return;} setShowPreview(true) } : handleLaunch}
          disabled={launching} className="btn-primary w-full text-base py-4">
